            ...
            if (args.length == 2) {
                if (args[0].equalsIgnoreCase("*")) {
                    ...
                    if (balancePlayerSender > sum1 * (Bukkit.getOnlinePlayers().size() - 1)) {
                        ...
                        long removeAmmount = sum1 * (Bukkit.getOnlinePlayers().size() - 1);
                        p.sendMessage(Main.prefix + "§3Du hast jedem Spieler §b" + sum1 + "§b " + Main.currency + "§3 überwiesen. §7(-" + removeAmmount + ")");
                        try {
                ...
                //Nicht Anfassen
                long sumPlayerSender = 0L;
                long sumPlayerTarget;
